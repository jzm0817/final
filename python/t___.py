...
class Config(object):
    ...
    """
    # 0.参数调整
    data_path = "D:/workspace/art/pic/stft_origin/"
    virs = "result"
    num_workers = 4  # 多线程
    img_size = 96 * 2  # 剪切图片的像素大小
    batch_size = 16 # 批处理数量
    max_epoch = 100   # 最大轮次
    lr1 = 2e-4  # 生成器学习率
    lr2 = 2e-4  # 判别器学习率
    ...
    # 1.模型保存路径
    save_path = 'D:/workspace/art/pic/res'  # opt.netg_path生成图片的保存路径
    # 判别模型的更新频率要高于生成模型
...
class NetG(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.Gene = nn.Sequential(
            nn.ConvTranspose2d(in_channels=opt.nz, out_channels=self.ngf * 16, kernel_size=4, stride=1, padding=0, bias =False),
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(in_channels=self.ngf * 1, out_channels=3, kernel_size=5, stride=3, padding=1, bias =False),
...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.save_every == 0:
            ...
            tv.utils.save_image(fix_fake_image.data[:opt.gen_num], "%s/%s.png" % (opt.save_path, epoch), normalize=True)

            torch.save(netd.state_dict(),  'D:/workspace/art/net/' + 'netd_{0}.pth'.format(epoch))
            torch.save(netg.state_dict(),  'D:/workspace/art/net/' + 'netg_{0}.pth'.format(epoch))
...
def generate(**kwargs):
    ...
    netd.load_state_dict(torch.load('D:/workspace/art/net/netd_' + str(opt.max_epoch - 1) + '.pth', map_location=map_location), False)
    netg.load_state_dict(torch.load('D:/workspace/art/net/netg_' + str(opt.max_epoch - 1) + '.pth', map_location=map_location), False)
    netd.to(device)
    netg.to(device)
